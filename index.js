...
var numberplateErrors = {
	1: "Unknown Format",
	2: "Invalid Characters",
	3: "Asterisk",
	4: "Q in Registration",
	5: "I but not Irish",
	6: "Z but not Irish or New Style",
	7: "New Style Z in first 2 characters",
...
var validate = function (input) {
	...
	if (!err) {
		...
		if (data["plate_format"] != null) {
			...
			switch (data["plate_format"]) {
				...
				case "1CT2D":
					data["plate"] =
						reg_array[0] + " " + reg_array[1] + reg_array[2];
					data["prefix"] = null;
					data["suffix"] = reg_array[0];
			...
			if (qflag && !err) {
				if (
					data["plate_format"] != "P1" &&
					data["plate_format"] != "P2" &&
					data["plate_format"] != "P3"
				) {
			...
			if (irishi && !err) {
				if (
					...
					data["plate_format"] == "P2" ||
					data["plate_format"] == "P3" ||
					data["plate_format"] == "S1" ||
					data["plate_format"] == "S2" ||
					data["plate_format"] == "S3" ||
					data["plate_format"] == "MILLENNIUM" ||
					data["plate_format"] == "1CT1D" ||
					data["plate_format"] == "1CT2D" ||
					data["plate_format"] == "1CT3D" ||
					data["plate_format"] == "1CT4D" ||
					data["plate_format"] == "1DT1C" ||
					data["plate_format"] == "2DT1C" ||
					data["plate_format"] == "3DT1C" ||
					data["plate_format"] == "4DT1C"
				) {
			...
			if (!err) {
				if (
					...
					if (parseInt(data["number"]) > 50) {
						data["year_of_issue_expiry"] = String(
							parseInt(year) + 1
						);
					} else {
	...
	// callback(err,data);
	return {
		err: numberplateErrors[err],
		data: data,
	};
};

var validateAsync = function (value) {
	return new Promise(function (resolve, reject) {
		var result = validate(value);
		if (result.err) {
			reject(numberplateErrors[result.err]);
		} else {
			resolve(result.data);
		}
	});
